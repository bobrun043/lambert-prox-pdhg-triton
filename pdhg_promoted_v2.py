"""Promoted canonical PDHG integration for the Lambert-Prox project.

The module has one mathematical oracle for all separable fidelity proxes:
``lambert_prox_torch_v1``.  The CUDA/Triton backends are promoted only
when the packaged validation evidence and source hashes verify. They are never
selected silently.  Requesting an unavailable backend raises an
explicit error.
...
DEFAULT_PROMOTION_MANIFEST = Path(__file__).with_name("promotion_manifest_v1.json")
...
def verify_promotion_manifest(manifest_path: str | Path = DEFAULT_PROMOTION_MANIFEST) -> dict[str, Any]:
    """Verify report status and bind the promoted package to exact source bytes.

    The original CUDA report did not embed source hashes. The packaged manifest
    therefore records a post-run provenance binding. This proves that the promoted
    package has not changed since assembly, but it is not a cryptographic proof
    that the original Colab process executed these exact bytes.
    """
    path = Path(manifest_path)
    if not path.exists():
        raise PromotionEvidenceError(f"promotion manifest not found: {path}")
    data = json.loads(path.read_text(encoding="utf-8"))
    if data.get("schema") != "lambert-prox-triton-promotion-v1":
        raise PromotionEvidenceError(f"unexpected promotion schema: {data.get('schema')!r}")
    base = path.parent
    report_meta = data.get("validation_report", {})
    report_path = base / report_meta.get("filename", "")
    if not report_path.exists():
        raise PromotionEvidenceError(f"validation report missing: {report_path}")
    if _sha256_file(report_path) != report_meta.get("sha256"):
        raise PromotionEvidenceError("validation report hash mismatch")
    report = json.loads(report_path.read_text(encoding="utf-8"))
    if report.get("schema") != "triton-oracle-validation-v2" or report.get("status") != "PASS":
        raise PromotionEvidenceError("validation report is not an accepted PASS report")
    tests = report.get("tests", [])
    if len(tests) < 47 or any(not bool(t.get("pass")) for t in tests):
        raise PromotionEvidenceError("validation report is incomplete or contains failures")
    env = report.get("environment", {})
    if not env.get("cuda_available") or not env.get("triton_importable"):
        raise PromotionEvidenceError("validation report lacks CUDA/Triton evidence")
    summary = report.get("summary", {})
    if int(summary.get("count", -1)) != len(tests) or int(summary.get("passed", -1)) != len(tests):
        raise PromotionEvidenceError("validation report summary is inconsistent")
    if float(summary.get("worst_scaled", float("inf"))) > 8.0e-5:
        raise PromotionEvidenceError("validation report exceeds the canonical tolerance")
    for filename, expected in data.get("sources", {}).items():
        p = base / filename
        if not p.exists() or _sha256_file(p) != expected:
            raise PromotionEvidenceError(f"promoted source hash mismatch: {filename}")
    return data
...
def resolve_backend(
    backend: Backend | str,
    *,
    elementwise_module: str = "triton_prox_canonical_v1",
    stencil_module: str = "triton_stencil_canonical_v1",
    promotion_manifest: str | Path = DEFAULT_PROMOTION_MANIFEST,
    ...
    evidence = verify_promotion_manifest(promotion_manifest)
    if not cuda:
    ...
    if requested == Backend.TRITON_ELEMENTWISE:
        ...
        report = BackendReport(
            ...
            promotion_manifest=str(Path(promotion_manifest)),
        )
...
def _elementwise_candidate_prox(mod: Any, kind: str, x: torch.Tensor, kty: torch.Tensor,
    ...
    if k == "poisson_log":
        return mod.prox_poisson_log_axpy(x, kty, y_obs, **kwargs)
    if k == "kl":
    ...
    *,
    ...
    if selected != Backend.TORCH:
        if x0.device.type != "cuda":
            raise BackendUnavailableError("promoted Triton backends require CUDA tensors")
        if x0.dtype != torch.float32:
            raise TypeError("promoted Triton backends are validated for float32 only")
...
__all__ = [
    "Backend", "BackendUnavailableError", "PromotionEvidenceError", "BackendReport", "PDHGInfo",
    "DEFAULT_PROMOTION_MANIFEST", "verify_promotion_manifest",
